function createSlider(mainBlock, rowBlock, previousButton, nextButton){
    ...
    for (var key in elementsList) {
      if(elementsList[key] == '[object HTMLImageElement]' || elementsList[key] == '[object HTMLDivElement]'){
        ...
        if(widthElement == undefined){
          widthElement = parseInt(getComputedStyle(elementsList[key]).width);
        }
      }
    }

    let slidesInWindow = parseInt(getComputedStyle(document.querySelector(mainBlock)).width) / widthElement;
    return {
      'counter': counter,
      'width': widthElement,
  ...
  let previousSlide = (blockWidth, elcounter, slidesShowBase) => {
    ...
    if(getComputedStyle(row).left != '0px'){
      let newLeft = parseInt(getComputedStyle(row).left) + blockWidth;
  ...
  let makeSlider = () => {
    ...
    window.onresize = function(event) {
      ...
      if(-maxLeft > parseInt(getComputedStyle(document.querySelector(rowBlock)).left)){
